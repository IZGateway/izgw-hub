...
`ApiKeyCredential` SHALL be a DynamoDB entity following Hub's single-table design. It SHALL extend `DynamoDbAudit` and be annotated with `@DynamoDbBean`. Its sort key SHALL be `{env}#{jti}`, where `env` is the environment name and `jti` is the UUID token identifier. The partition key is `ApiKeyCredential`, set automatically by `DynamoDbRepository` using `entityClass.getSimpleName()`.

Required fields:
- `jti` — String; the JWT `jti` claim; unique credential identifier
- `env` — String; the environment name (e.g., `Production`, `Onboarding`, `Development`)
...
- **WHEN** Config Console writes an `ApiKeyCredential` record with `status = active` using sort key `Production#<jti>` (partition key `ApiKeyCredential`)
...
The `sortKey` SHALL be formatted as `{env}#{jti}` where `{env}` is the runtime environment string and `{jti}` is the credential's UUID. The partition key is always `ApiKeyCredential` (the entity class simple name), set by `DynamoDbRepository`. This format is consistent with Hub's single-table design and allows Hub to scope lookups to the current environment by prefix.
...
- **THEN** its `sortKey` value is `Production#018f4e2a-5678-7abc-8def-000000000002` and its partition key is `ApiKeyCredential`

#### Scenario: Sort key for non-Production environments
- **WHEN** an `ApiKeyCredential` is created for `env = Onboarding` and `jti = 018f4e2a-5678-7abc-8def-000000000003`
- **THEN** its `sortKey` value is `Onboarding#018f4e2a-5678-7abc-8def-000000000003` and its partition key is `ApiKeyCredential`
