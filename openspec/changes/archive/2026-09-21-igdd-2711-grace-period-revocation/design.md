...
Each run logs a structured completion record with counts; failures are logged at ERROR. A CloudWatch log-based alarm fires when either (a) an error is logged by the job, or (b) no successful-completion record is seen within an expected window (missed run). The [operations runbook](https://izgateway.atlassian.net/wiki/spaces/IGDD/pages/1011417090/Runbook+Grace-Period+Revocation+Job) documents the manual remediation: confirm Hub health, and if needed run the manual revoke through Config Console (`DELETE /api/apikeys/:jti`) for any key whose `graceExpiresAt` has passed.
