# Spec: API Documentation (Swagger UI)

**Component:** `SwaggerUiVersionConfig` (izgw-core), Springdoc `SwaggerUiConfigProperties` (IGDD-3084; see IGDD-2976)  
**Implemented in:** `gov.cdc.izgateway.configuration.SwaggerUiVersionConfig` (izgw-core), `src/main/resources/application.yml` (`springdoc` block), `gov.cdc.izgateway.configuration.SwaggerUiVersionIntegrationTests` (regression test)  
**Related specs:** `../access-control/spec.md`  

---

## Purpose

Hub serves Swagger UI and its OpenAPI document via Springdoc at `/swagger/ui.html`,
`/swagger/api-docs`, and `/swagger/swagger-ui/**`. At startup, `SwaggerUiVersionConfig`
in izgw-core (`gov.cdc.izgateway.configuration`, picked up by Hub's default component
scan) detects the `org.webjars:swagger-ui` version on the classpath via
`org.webjars.WebJarVersionLocator` and force-sets it on Springdoc's
`SwaggerUiConfigProperties`, so the UI keeps working across `izgw-bom` bumps of the
webjar with no hand-pinned `springdoc.swagger-ui.version` in `application.yml`
(IGDD-3084; previously fixed per-service in izgw-transform under IGDD-2976). The
existing admin-only `/swagger/**` rule in izgw-core's `AccessControlValve.isSwagger()`
is unchanged.
