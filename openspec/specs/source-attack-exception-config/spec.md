# Spec: Source-Attack Exception Config

**Component:** `SourceAttackExceptionController`, `SourceAttackExceptionRecord`, `SourceAttackExceptionRepository`, `NewModelHelper` (IGDD-2805)  
**Implemented in:** `gov.cdc.izgateway.hub.SourceAttackExceptionController`, `gov.cdc.izgateway.dynamodb.model.SourceAttackExceptionRecord`, `gov.cdc.izgateway.dynamodb.repository.SourceAttackExceptionRepository`, `gov.cdc.izgateway.hub.service.accesscontrol.NewModelHelper`  
...
A DynamoDB-backed, per-sender exemption from source-attack auto-lockout (IGDD-2805),
administered without a deploy through the admin-only `/rest/sourceAttackExceptions` REST API
(`SourceAttackExceptionController`, `@RolesAllowed(ADMIN)`). Records are
`SourceAttackExceptionRecord` entries (sender, reason, audit fields), cached in
`NewModelHelper` and refreshed on the access-control refresh period. An exception suppresses
only the deny-list add — the flagged message is still rejected with `SecurityFault` code `61`.
